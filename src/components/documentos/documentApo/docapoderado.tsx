...
import {
  api_getDocumentsByCourse,
  api_getDocumentsByEstablishment,
  api_getDocumentsByUserDestinity,
                  ...
                  {displayedDocuments.map((doc, index) => (
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {doc.attributes.descriptionDoc}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {doc.attributes.userId.data.attributes.firstname +
                          " " +
                          doc.attributes.userId.data.attributes.first_lastname}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        ...
                        {doc.attributes.document?.data?.length === 1 ? (
                          <button
                            ...
                            onClick={() => {
                              handleDownload(
                                getFile(
                                  doc.attributes.document.data[0].attributes.url
                                ),
                                doc.attributes.document.data[0].attributes.name
                              );
                            }}
                            disabled={loading}
                          >
                          ...
                          >
                            ...
                            <Menu.Items className="mt-2 w-56 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                              {" "}
                              <div className="px-1 py-1">
                                {doc.attributes.document?.data?.map(
                                  (archivo, i) => (
                                    <Menu.Item key={i}>
                                      {({ active }) => (
                                        <button
                                          ...
                                          onClick={() => {
                                            handleDownload(
                                              getFile(archivo.attributes.url),
                                              archivo.attributes.name
                                            );
                                          }}
                                          disabled={loading}
                                        >
