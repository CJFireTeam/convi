...
import {
  api_getActiveAdministrador,
  api_getAllAdministrador,
  api_getBlockAdministrador,
  api_getpendientAdministrador,
  api_putBlockedUser,
  api_putUserAdmin,
  api_role,
  api_updateUser,
...
export default function VerAdministradores() {
  ...
  const filteredAdmins = totalAdmin.filter((admin) => {
    const nameMatch = `${admin.firstname} ${admin.first_lastname}`
      .toLowerCase()
    ...
    const statusMatch =
      adminFilter === "all"
        ? true
        : adminFilter === "active"
        ? admin.confirmed && !admin.blocked
        : adminFilter === "pending"
        ? !admin.confirmed
        : adminFilter === "blocked"
        ? admin.blocked
        : adminFilter === "with_establishment"
        ? admin.establishment !== null
        : adminFilter === "without_establishment"
        ? admin.establishment === null
        : true;
  ...
  // Añadir esta función para eliminar
  const handleDeleteAdmin = async () => {
    if (!selectedAdminId) return;

    try {
      setLoading2(true);
      await api_updateUser(selectedAdminId, { eliminado: true });
    ...
    <>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-[700px] w-[102vw] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              <SquareUser className="h-5 w-5 inline-block" /> Total de
              Administradores
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="my-4 space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="with_establishment">
                  Con establecimiento
                </SelectItem>
                <SelectItem value="without_establishment">
                  Sin establecimiento
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            {currentAdmins.map((admin, i) => (
              <TooltipProvider key={admin.id}>
                <Tooltip>
                      ...
                      <AccordionItem value={`item-${i}`}>
                        <AccordionTrigger>
                          <div className="flex items-center justify-between w-full">
                            <span>
                              {admin.firstname} {admin.first_lastname}
                            </span>
                            <div className="flex items-center space-x-2">
                              <span
                          ...
                          <div className="grid grid-cols-2 gap-2 p-4">
                            {admin.establishment ? (
                              <>
                                ...
                                <Label>
                                  Establecimiento: {admin.establishment.name}
                                </Label>
                                <Label>
                                  Dirección: {admin.establishment.address}
                                </Label>
                                <Label>
                                  Comuna: {admin.establishment.Comuna}
                                </Label>
                                <Separator className="my-4 col-span-2" />
                              </>
                            ) : (
                              <>
                                <Label className="col-span-2 text-red-500">
                            ...
                            <Label>
                              Nombre completo: {admin.firstname}{" "}
                              {admin.secondname} {admin.first_lastname}{" "}
                              {admin.second_lastname}
                            </Label>
                            <Label>
                              Estado:{" "}
                              {admin.confirmed
                  ...
                  <TooltipContent>
                    {!admin.establishment
                      ? "Este administrador no tiene establecimiento asignado"
          ...
          {editingAdmin && (
            ...
            >
              <div className="grid gap-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="first_lastname" className="text-right">
                    Primer Apellido
                  </Label>
                  <Input
                    id="first_lastname"
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="comuna" className="text-right">
                    Comuna
                  </Label>
                  <Select
                    name="comuna"
                  ...
                  <Select
                    ...
                    value={
                      selectedEstablishment === null
                        ? "null"
                        : selectedEstablishment?.toString()
                    }
                    onValueChange={(value) =>
      ...
      <Dialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            {loading2 ? <Button variant="destructive" disabled>
              Cargando
            </Button> : <Button variant="destructive" onClick={handleDeleteAdmin}>
              Confirmar
            </Button>}
          </DialogFooter>
        </DialogContent>
